...
pub fn solve(
    ...
    // Headline score uses the *real* routed-trace loop length (the score path can
    // afford the router; the inner loop used the fixed-pad surrogate).
    ...
    const bd = breakdownWith(parts, params, score, routed.weighted);
...
pub fn scorePoses(
    ...
    score.loop_mm = routed.raw;
    return breakdownWith(prep.parts, params, score, routed.weighted);
}
...
fn prepare(
    ...
    // Per-part set of footprint pad numbers whose pin function is an auxiliary
    // rail tie (EN, PGFB, FB, SET, …) rather than a true supply pin. Only hubs
    // need it — only a hub's pads seed a decoupling loop's target set — so a
    // bypass cap hugs the real supply pads, not an incidental strap (e.g. the
    // LT3045's EN/UV + PGFB pins tied to V_IN). Passives get an empty set; the
    // per-key cache avoids re-reading a pinout shared by repeated parts.
    var aux_cache = std.StringHashMap(std.StringHashMapUnmanaged(void)).init(arena);
    const aux_pads = try arena.alloc(std.StringHashMapUnmanaged(void), instances.len);
    for (instances, 0..) |inst, i| {
        if (parts[i].kind != .hub) {
            aux_pads[i] = .empty;
            continue;
        ...
        const gop = try aux_cache.getOrPut(key);
        if (!gop.found_existing) gop.value_ptr.* = try loadAuxPads(arena, project_dir, key);
        aux_pads[i] = gop.value_ptr.*;
    }
    ...
    const built = try buildSprings(arena, nets, parts, &idx_of, aux_pads, explicit_pin);
...
fn objectiveCost(
    ...
    return hpwlOf(parts, idx_of, nets) + params.loop_w * weightedLoop(parts, loops) +
...
/// `aux` flags a hub pad whose pin function is an auxiliary rail tie (EN, PGFB,
/// FB, …) rather than a true supply pin — excluded from a decoupling loop's
/// target set when real supply pins share the net (see `hugToHub`).
const Endpoint = struct { idx: usize, px: f64, py: f64, pw: f64, ph: f64, is_hub: bool, aux: bool = false, pin: []const u8 = "" };
...
fn buildSprings(
    ...
    parts: []const Part,
    idx_of: *std.StringHashMap(usize),
    aux_pads: []const std.StringHashMapUnmanaged(void),
    ...
    for (nets, 0..) |net, net_i| {
        ...
        for (net.pins) |pr| {
            ...
            const is_aux = aux_pads[i].contains(pr.pin);
            if (is_hub) {
                if (hub_idx) |h| {
                    if (h != i) multi_hub = true;
                } else hub_idx = i;
            }
            try eps.append(arena, .{ .idx = i, .px = pad.x, .py = pad.y, .pw = pad.w, .ph = pad.h, .is_hub = is_hub, .aux = is_aux, .pin = pr.pin });
        }
        if (eps.items.len < 2) continue;

        const single_hub = hub_idx != null and !multi_hub;
        if (single_hub) {
            try hugToHub(arena, &springs, &loops, eps.items, hub_idx.?, gnd, explicit_pin, @intCast(net_i));
        } else {
...
fn hugToHub(
    ...
    // Prefer the hub's *true supply* pins: an EN / PG / PGFB / FB-type pin that
    // merely straps to the rail for configuration is not what a bypass cap hugs.
    // Fall back to all of the hub's pads on the net when none qualify (or the
    // part has no pinout), preserving the prior behaviour.
    var any_supply = false;
    for (eps) |e| {
        if (e.idx == hub and !e.aux) {
            any_supply = true;
            break;
        }
    }
    ...
    var hx: f64 = 0;
    var hy: f64 = 0;
    // Track the lowest-numbered kept supply pad — the default pinned target for a
    // cap that doesn't name an explicit pin.
    var def_rect: ?PadRect = null;
    var def_pin: []const u8 = "";
    for (eps) |e| {
        if (e.idx != hub) continue;
        if (any_supply and e.aux) continue;
        const pr = PadRect{ .x = e.px, .y = e.py, .w = e.pw, .h = e.ph };
        try hub_pwr.append(arena, pr);
        hx += e.px;
        hy += e.py;
    ...
    const default_pin_rect = def_rect.?;
...
/// Total half-perimeter wirelength over signal nets (ground nets skipped — they
/// are planes). The hot part of the inner-loop objective, so it's its own fn.
fn hpwlOf(parts: []const Part, idx_of: *std.StringHashMap(usize), nets: []const FlatNet) f64 {
    var hpwl: f64 = 0;
    for (nets) |net| {
        if (isGroundName(shortName(net.name))) continue;
...
fn scoreLayout(
    ...
    return .{
        .hpwl_mm = hpwlOf(parts, idx_of, nets),
...
test "buildSprings excludes aux rail pins (PGFB/EN) from a decoupling loop's hub target" {
    var arena_inst = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_inst.deinit();
    const arena = arena_inst.allocator();

    // U1 (LT3045-like): three pads on V_12V — two real inputs (1,2, left) and
    // one config strap (6 = PGFB, far right) — plus a ground pad (8). C1 is the
    // input bypass cap on V_12V → it must hug the inputs, not PGFB.
    const u1_pads = [_]geometry.Pad{
        .{ .number = "1", .x = -1.0, .y = -1, .w = 0.3, .h = 0.3 },
        .{ .number = "2", .x = -0.5, .y = -1, .w = 0.3, .h = 0.3 },
        .{ .number = "6", .x = 1.5, .y = -1, .w = 0.3, .h = 0.3 },
        .{ .number = "8", .x = 0.0, .y = -1, .w = 0.3, .h = 0.3 },
    };
    const c1_pads = [_]geometry.Pad{
        .{ .number = "1", .x = 0.0, .y = 0.0, .w = 0.3, .h = 0.3 },
        .{ .number = "2", .x = 0.0, .y = 0.3, .w = 0.3, .h = 0.3 },
    };
    var parts = [_]Part{
        .{ .ref_des = "U1", .kind = .hub, .hw = 2, .hh = 1, .pads = &u1_pads, .fallback = false },
        .{ .ref_des = "C1", .kind = .passive, .hw = 0.5, .hh = 0.3, .pads = &c1_pads, .fallback = false, .value = "4.7uF" },
    };
    const n_vin = [_]export_kicad.FlatPin{
        .{ .ref_des = "U1", .pin = "1" }, .{ .ref_des = "U1", .pin = "2" },
        .{ .ref_des = "U1", .pin = "6" }, .{ .ref_des = "C1", .pin = "1" },
    };
    const n_gnd = [_]export_kicad.FlatPin{ .{ .ref_des = "U1", .pin = "8" }, .{ .ref_des = "C1", .pin = "2" } };
    const nets = [_]FlatNet{ .{ .name = "V_12V", .pins = &n_vin }, .{ .name = "GND", .pins = &n_gnd } };

    var idx = std.StringHashMap(usize).init(arena);
    try idx.put("U1", 0);
    try idx.put("C1", 1);

    // Mark U1 pad "6" (PGFB) auxiliary; pads 1 & 2 are real supply pins.
    var aux_u1: std.StringHashMapUnmanaged(void) = .empty;
    try aux_u1.put(arena, "6", {});
    const aux_pads = [_]std.StringHashMapUnmanaged(void){ aux_u1, .empty };
    const no_pins = [_][]const u8{ "", "" }; // no (placement-order …) overrides

    const built = try buildSprings(arena, &nets, &parts, &idx, &aux_pads, &no_pins);
    try testing.expectEqual(@as(usize, 1), built.loops.len);
    // The loop targets only the two supply pads — PGFB (x = 1.5) is excluded.
    try testing.expectEqual(@as(usize, 2), built.loops[0].hub_pwr.len);
    for (built.loops[0].hub_pwr) |p| try testing.expect(p.x < 0);
    // …and the power leg is *pinned* to the lowest-numbered supply pad (pin 1,
    // x = -1.0), not re-chosen each eval — the fix for the score-jump on a nudge.
    try testing.expectEqual(@as(f64, -1.0), built.loops[0].hub_pwr_pin.x);
    // The ground return is pinned to the hub GND pad (pin 8, y = -1) too, so the
    // full loop measures cap_gnd → that pad, not a flat constant.
    try testing.expectEqual(@as(f64, -1.0), built.loops[0].hub_gnd_pin.y);

    // An explicit `(near 2 "C1")` overrides the default to pin 2 (x = -0.5).
    const pin2 = [_][]const u8{ "", "2" };
    const built_ex = try buildSprings(arena, &nets, &parts, &idx, &aux_pads, &pin2);
    try testing.expectEqual(@as(f64, -0.5), built_ex.loops[0].hub_pwr_pin.x);

    // Fallback: when every V_12V hub pad is an aux strap, keep them all rather
    // than dropping the rail's loop entirely.
    var aux_all: std.StringHashMapUnmanaged(void) = .empty;
    try aux_all.put(arena, "1", {});
    try aux_all.put(arena, "2", {});
    try aux_all.put(arena, "6", {});
    const aux_pads_all = [_]std.StringHashMapUnmanaged(void){ aux_all, .empty };
    const built_fb = try buildSprings(arena, &nets, &parts, &idx, &aux_pads_all, &no_pins);
    try testing.expectEqual(@as(usize, 3), built_fb.loops[0].hub_pwr.len);
}

test "isAuxRailPin distinguishes supply pins from control/feedback straps" {
    // Real supply pins stay non-aux …
    try testing.expect(!isAuxRailPin("IN_1"));
    try testing.expect(!isAuxRailPin("IN_2"));
    try testing.expect(!isAuxRailPin("OUT"));
    try testing.expect(!isAuxRailPin("OUTS"));
    try testing.expect(!isAuxRailPin("VDD"));
    try testing.expect(!isAuxRailPin("VBUS"));
    // … while config / feedback / sense straps are auxiliary.
    try testing.expect(isAuxRailPin("EN/UV"));
    try testing.expect(isAuxRailPin("PGFB"));
    try testing.expect(isAuxRailPin("PG"));
    try testing.expect(isAuxRailPin("ILIM"));
    try testing.expect(isAuxRailPin("SET"));
    try testing.expect(isAuxRailPin("FB"));
}
