...
pub const scope_form_docs = blk: {
    ...
    t[@intFromEnum(ScopeForm.design_rules)] = .{ .scope = tl, .doc = .{
        .syntax = "(design-rules [(clearance MM)] [(min-drill MM)] [(mask-margin MM)] [(copper-edge MM)] " ++
            "[(hole-to-hole MM)] [(min-annular MM)] [(track-width MM)] [(via DIA DRILL)])",
        .summary = "Board-level DEFAULT design rules (all sub-forms optional; mm): (clearance) copper-to-copper " ++
            ...
            "(copper radius − drill radius); (track-width) the default routed trace width; (via DIA DRILL) " ++
            "the default via copper diameter + drill. The last three seed the autorouter's geometry — an " ++
            "explicit query/panel override still wins for interactive routing, but the fab gate judges the " ++
            "board against these authored rules. All are global defaults — a per-net (net-class …) still " ++
            ...
            "hole-to-hole 0.25, min-annular 0.1, track-width 0.127, via 0.4 / 0.2), so a design with no form " ++
            "is unchanged.",
    } };
