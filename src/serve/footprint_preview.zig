...
pub fn listFootprints(w: anytype, ctx: *Handler) HandlerError!void {
    ...
    while (try iter.next()) |entry| {
        if (entry.kind == .file and std.mem.endsWith(u8, entry.name, ".sexp")) {
            ...
            const dims = footprintPadBounds(ctx.allocator, file_content);
            const has_model = blk: {
                var mcfg = export_kicad.loadModelConfig(ctx.allocator, ctx.project_dir);
                defer mcfg.deinit();
                if (mcfg.get(fname)) |c| {
                    if (c.model != null) break :blk true;
                }
                break :blk footprint_mod.findModelFile(ctx.allocator, ctx.project_dir, fname, fname) != null;
            };
            if (dims) |d| {
                try w.print("<tr><td>{s}</td><td>{d:.2} x {d:.2}</td>", .{ fname, d.w, d.h });
            } else {
