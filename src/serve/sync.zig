...
fn matchInstance(d: *DiffContext, inst: export_kicad.FlatInstance, w: anytype, first: *bool, ops_emitted: *u32) !?BoardFp {
    ...
    // Last resort before treating the instance as new: a board fp whose
    // KiCad-internal uuid equals this instance's canopy_uuid. Re-stamp the
    // canopy_uuid property so the next sync UUID-matches directly, and adopt
    // in place (preserving the user's placement) instead of adding a
    // duplicate at the origin.
    if (pickByKicadUuid(inst.uuid, d.by_kicad_uuid, d.matched_uuids)) |m| {
        const target = opTargetUuid(m);
        if (try emitOpUnlessApplied(d, w, first, OP_SET_FIELD, .{
            .{ "uuid", target },
            .{ "field", FIELD_CANOPY_UUID },
