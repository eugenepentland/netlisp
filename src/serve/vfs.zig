...
/// `.layout` files are binary PCB data regenerated by `build` — read-only.
...
test "checkAcl denies writes to layout files" {
    // spec: serve/vfs - denies writes to .layout files
    try std.testing.expectError(error.PermissionDenied, checkAcl("src/foo.layout", .write));
}
