  ...
  document.querySelectorAll('.'+buttonClass).forEach(function(btn){
    ...
    function save(){
      ...
      function next(){
        if(!ok||i>=refs.length){
          btn.disabled=false;
          if(ok){btn.textContent='Saved';input.classList.add('bom-saved');setTimeout(function(){btn.textContent='Save';input.classList.remove('bom-saved');},2000);}
          else{btn.textContent='Error';input.classList.add('bom-error');}
          return;
        }
        var body={ref:refs[i]};body[propName]=newVal;
        fetch(endpoint+'/'+SCHEMATIC_SLUG,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)})
          .then(function(r){return r.json();})
