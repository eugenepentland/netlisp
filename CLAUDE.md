...
- **Cross-probing**: `/pcb-layout/:name?focus=REF` (or `#REF`) zooms/flashes a part (leaf-matching like `?refs=`); PCB sidebar rows link "Show in schematic →" (`#comp-REF` scroll+flash), schematic component detail links "Locate on PCB →". **Two-window live sync**: with `/pcb-layout/<name>` and `/schematics/<name>` open in separate tabs/windows of the same browser (the KiCad two-monitor workflow), clicking a part on the board highlights the component on the schematic page and vice versa — a `BroadcastChannel("netlisp-xprobe")` bridge in `pcb_board.js`/`schematic_viewer.js` (messages `{from, design, ref}`, filtered by design name, refs resolved exact-then-leaf; no server round-trip).
