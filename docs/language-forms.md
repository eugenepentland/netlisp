...
| `(net-class "name" [(width MM)] [(clearance MM)] [(via DIA DRILL)] (nets "A" "B"…))` | D·· | Routing geometry for the named nets: trace width, copper clearance, and via size (diameter + drill) in mm. Omitted numbers keep the router defaults; net names match the flattened netlist case-insensitively. The first class naming a net wins. Repeat the form for more classes (e.g. a wide "power" class and a tight "rf" class). |
