...
| `(stackup N [(plane IDX "NET")…])` | D·· | Declare the board's copper stack: N total copper layers (1-based, 1 = top/F.Cu, N = bottom/B.Cu); each (plane IDX "NET") makes layer IDX a solid plane carrying NET, all other layers are routed signal layers. `(stackup 2)` is a plain 2-layer board with no planes — ground/power are routed as copper like any other net. Without the form the router keeps its legacy implicit model (4 layers with assumed GND+PWR inner planes). |
